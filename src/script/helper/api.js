export default {
  ...
  errorHandle(xhr, callback, eventKey, eventType) {
    let msg = '';
    const eventOpts = {};
    if (eventType) eventOpts[Config.qiangRuanMap.api_type] = eventType;
    if (xhr.status === 401 || xhr.status === 403) {
      if (eventKey) {
        eventOpts[Config.qiangRuanMap.api_content] = '会话已过期';
        Utils.qrReport(eventKey, eventOpts);
      }
      if (WelabApp.isApp) {
        $core.BeanAlert({
          text: '您的会话已过期，请重新登录',
      ...
      try {
        ...
        if (rsp.message && rsp.result) {
          const keys = Object.keys(rsp.result);
          if (keys.length > 0) {
            msg = rsp.result[keys[0]];
          } else {
            msg = rsp.message;
          }
        } else if (rsp.message) {
          ...
          const values = [];
          Object.keys(rsp.errors).map((key) => {
            values.push(rsp.errors[key]);
            return key;
          });
          msg = values.join(',');
        }
    ...
    if (msg) {
      if (eventKey) {
        eventOpts[Config.qiangRuanMap.api_content] = msg;
        Utils.qrReport(eventKey, eventOpts);
      }
      if (typeof callback === 'function') {
        callback.apply(this, [msg]);
      } else {
        $core.ToastAlert.info(msg);
      }
    }
  },
  // API上传助手
